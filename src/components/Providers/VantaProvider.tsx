...
export function VantaProvider({ children }: { children: React.ReactNode }) {
  ...
  const [vantaInstance, setVantaInstance] = useState<unknown>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const waitForGlobals = async (): Promise<
        ((opts: Record<string, unknown>) => { destroy: () => void }) | null
      > => {
        const maxAttempts = 40;
        const delayMs = 100;
        for (let i = 0; i < maxAttempts; i += 1) {
          const windowAny = window as unknown as Record<string, unknown>;
          const hasThree = Boolean(windowAny?.THREE);
          const vanta = windowAny?.VANTA as Record<string, unknown> | undefined;
          const birds = vanta?.BIRDS;
          if (hasThree && birds)
            return birds as (opts: Record<string, unknown>) => {
              destroy: () => void;
            };
    ...
    return () => {
      ...
      try {
        if (
          localInstance &&
          typeof localInstance === "object" &&
          localInstance !== null &&
          "destroy" in localInstance
        ) {
          (localInstance as { destroy: () => void }).destroy();
        }
        if (
          vantaInstance &&
          vantaInstance !== localInstance &&
          typeof vantaInstance === "object" &&
          vantaInstance !== null &&
          "destroy" in vantaInstance
        ) {
          (vantaInstance as { destroy: () => void }).destroy();
        }
      } catch {
        // no-op cleanup
      }
